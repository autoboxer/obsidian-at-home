...
<template>
  <aside
    class="link-inspector"
    data-ui-region="context-panel"
    :data-context-view="uiState.inspectorTab"
  >
    <header class="inspector-header">
      ...
      <button
        ...
        title="Hide note context"
        @click="uiState.contextOpen = false"
      >
    ...
    <div class="inspector-tabs">
      <button
        type="button"
        :class="{ active: uiState.inspectorTab === 'links' }"
        @click="uiState.inspectorTab = 'links'"
      >
        <AppIcon name="link" :size="14" /> Links
      </button>
      <button
        type="button"
        :class="{ active: uiState.inspectorTab === 'info' }"
        @click="uiState.inspectorTab = 'info'"
      >
    ...
    <div v-if="activeNote" class="inspector-scroll">
        ...
        <section class="connection-section backlink-section">
          ...
          <div v-if="backlinks.length" class="backlink-list">
            <button
              ...
              :key="`${backlink.note.id}-${backlink.link.index}`"
              type="button"
              class="backlink-card"
              @click="selectNote( backlink.note.id )"
            >
      ...
      <template v-else>
        <section class="note-info-card">
          <div class="info-row">
            <span>Created</span><strong>{{ formatDate( activeNote.createdAt ) }}</strong>
          </div>
          <div class="info-row">
            <span>Updated</span><strong>{{ formatDate( activeNote.updatedAt ) }}</strong>
          </div>
          <div class="info-row">
        ...
        <section class="info-tags">
          <div class="connection-heading">
            <span><AppIcon name="tag" :size="14" /> Tags</span><small>{{ activeNote.tags.length }}</small>
          </div>
